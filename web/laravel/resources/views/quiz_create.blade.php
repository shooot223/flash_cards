...
<form method="POST" action="{{ route('quiz.confirm') }}" class="quizForm">
    ...
    <div class="formGroup">
        ...
        <div id="questionsContainer">
            @for ($i = 0; $i < 5; $i++)
                <div class="questionBlock" style="margin-top:16px; padding:16px; border:1px solid #ddd; border-radius:8px;">
                    ...
                    <textarea id="q_{{ $i }}" name="questions[{{ $i }}][question]" class="formInput" required></textarea>

                    <label class="formLabel" for="a_{{ $i }}">答え</label>
                    <input type="text" id="a_{{ $i }}" name="questions[{{ $i }}][answer]" class="formInput" required>

                    <label class="formLabel">その他選択肢</label>
                    <input type="text" name="questions[{{ $i }}][choices][]" class="formInput" placeholder="選択肢1">
                    <input type="text" name="questions[{{ $i }}][choices][]" class="formInput" placeholder="選択肢2">
                    <input type="text" name="questions[{{ $i }}][choices][]" class="formInput" placeholder="選択肢3">
                </div>
            @endfor
        </div>
    </div>
...
<script>
    ...
    addQuestionBtn.addEventListener('click', () => {
        ...
        wrapper.innerHTML = `
            ...
            <textarea name="questions[${idx}][question]" class="formInput" required></textarea>

            <label class="formLabel">答え</label>
            <input type="text" name="questions[${idx}][answer]" class="formInput" required>
